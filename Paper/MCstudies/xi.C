void xi(const char *mc="1epos", const string centr="1030"){
  const int nV0cent = 9;
  ...
  TFile *fvzero = new TFile(Form("out%s_spd%s_vzero.root",mc,centr.c_str()));
  TFile *fzdc = new TFile(Form("out%s_spd%s_zdc.root",mc,centr.c_str()));
  TFile *fall = new TFile(Form("out%s_spd.root",mc));
