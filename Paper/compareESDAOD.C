...
void compareESDAOD(Double_t lMultBoundLo = 0.0,
  ...
  TString particle = "AntiLambda"){

    TString folder = "V0Analysis";
    //--------- spectra files ------------
    TFile* resaod = TFile::Open(Form("%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_AOD.root",folder.Data(), particle.Data(), lMultBoundLo, lMultBoundHi, lEEBoundLo, lEEBoundHi), "READ");
    TFile* resesd = TFile::Open(Form("%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_ESD.root",folder.Data(), particle.Data(), lMultBoundLo, lMultBoundHi, lEEBoundLo, lEEBoundHi), "READ");
    //TFile* resaod_ptint = TFile::Open(Form("%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_AODPtInt.root",folder.Data(), particle.Data(), lMultBoundLo, lMultBoundHi, lEEBoundLo, lEEBoundHi), "READ");
    //TFile* resesd_ptint = TFile::Open(Form("%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_ESDPtInt.root",folder.Data(), particle.Data(), lMultBoundLo, lMultBoundHi, lEEBoundLo, lEEBoundHi), "READ");
    
    //TH1D* aodsp_ptint = (TH1D*)resaod_ptint->Get("lInvMassReal/lInvMassRealRawData/fHistPtRaw");
    //TH1D* esdsp_ptint = (TH1D*)resaod_ptint->Get("lInvMassReal/lInvMassRealRawData/fHistPtRaw");
    ...
    TH1D* aodeff = (TH1D*)resaod->Get("fHistPureEfficiency");
    TH1D* esdeff = (TH1D*)resesd->Get("fHistPureEfficiency");
    ...
    /*TH1D* hratio_ptint = doratio(aodsp_ptint,esdsp_ptint);
    TH1D* h = new TH1D("h", ";#it{p}_{T} (GeV/#it{c})", 1, 0., 6.5);
    for(int b = 1; b<=hratio_ptint->GetNbinsX(); b++){
        h->SetBinContent(1,hratio_ptint->GetBinContent(1));
        h->SetBinError(1,hratio_ptint->GetBinError(1));
    }
    h->SetMarkerStyle(33);
    h->SetMarkerSize(3.7);
    ...
    */
    ...
    hratio->GetXaxis()->SetRangeUser(0.4,8.);
    hratio->Draw();
    //h->Draw("SAME EP");

    //l->AddEntry(h,"#it{p}_{T} integrated","EP");
    l->AddEntry(hratio,"#it{p}_{T} differential","EP");
    l->SetTextSize(0.04);
    l->Draw("SAME");
    
    TLine* l5p = new TLine(0.4,1.05,8.,1.05);
    l5p->SetLineColor(kRed);
    l5p->SetLineStyle(9);
    
    TLine* l5m = new TLine(0.4,.95,8.,.95);
    l5m->SetLineColor(kRed);
    l5m->SetLineStyle(9);

    l5p->Draw("SAME");
    l5m->Draw("SAME");
    xlabel-> DrawLatex(0.75, 0.78, Form("#bar{#Lambda}"));
    ...
    hratioeff->GetXaxis()->SetRangeUser(0.4,8.);
    hratioeff->Draw();
    hratioeff->SetMarkerSize(1.7);
    hratioeff->GetXaxis()->SetTitle("#it{p}_{T} (GeV/#it{c})");
    xlabel-> DrawLatex(0.75, 0.78, Form("#bar{#Lambda}"));

    TLine* l5pe = new TLine(0.4,1.05,8.,1.05);
    l5pe->SetLineColor(kRed);
    l5pe->SetLineStyle(9);
    
    TLine* l5me = new TLine(0.4,.95,8.,.95);
