...
Bool_t AliV0Module::CheckITSTOF(ULong64_t lPosTrackStatus, ULong64_t lNegTrackStatus, Int_t lPosTOFBCID, Int_t lNegTOFBCID){
    ...
    if ((lPosTrackStatus & AliVTrack::kITSrefit) || (lNegTrackStatus & AliVTrack::kITSrefit) )
...
void AliV0Module::DoAnalysis() {
    ...
    ********************************************************/
    ...
    for(Int_t ibin = 0; ibin<fptbinnumb; ibin++) {
        ...
        else lHistoFullV0[ibin]->Fit(fgausname,"QREM0");
    ...
    fileMC->cd("PWGLF_StrVsMult_MC");

    TList* v0listMC  = (TList*)fileMC->FindObjectAny("cList");
    //- - - TList* v0listMC  = (TList*)fileMC->FindObjectAny("cListNew");
    TTree *lTreeMC;
    TH1F* fHistMultiplicityBeforeTrigSelMC;
    lTreeMC = (TTree*)fileMC->FindObjectAny("fTreeV0");
    ...
    for(Long_t icand = 0; icand<lNCandidatesMC; icand++) {
        ...
        if(TMath::Abs(lRap)<fRapidityBoundary &&
            ...
            if ( lPrimaryStatus == 1) {
                ...
                if ( fWhichParticle == "Lambda"     ) lProtonMomentum[lWeAreAtBin]->Fill( TMath::Sqrt( lPosPx*lPosPx + lPosPy*lPosPy ) );
                if ( fWhichParticle == "AntiLambda" ) lProtonMomentum[lWeAreAtBin]->Fill( TMath::Sqrt( lPosPx*lPosPx + lPosPy*lPosPy ) );
...
//== Variables for holding peak position, width ==============
   ...
   // === for systematics due to Xi spectra
    ...
    if( fFDSwitch != "NoFD" && fWhichParticle != "K0Short") {
        ...
        if(fListName == "WithV0refitAndImprovedDCA") fListName = "withV0refitAndImprovedDCA";
        fileMCFD->cd(Form("PWGLF_StrVsMult%s_MC",fListName.Data()));
        TList* v0listMCFD     = (TList*)fileMCFD->FindObjectAny(Form("cList%s",fListName.Data()));
        TTree* lTreeMCFD      = (TTree*)fileMCFD->FindObjectAny("fTreeV0");
        TTree* lTreeEventMCFD = (TTree*)fileMCFD->FindObjectAny("fTreeEvent");
        ...
        for(Int_t ipoint = 0; ipoint<fptbinnumb; ipoint++) {
            for(Int_t ixb = 0; ixb<xibinnumb; ixb++) {
                if(fG3FlukaCorrOn){lFeedDownEfficiencyGFCorrected[ipoint][ixb] = lFeedDownEfficiency[ipoint][ixb]*(fitGeant3FlukaCorr->Eval( lProtonMomentum[ipoint]->GetMean() ) ) ;
                ...
                lFeedDownEfficiencyGFCorrected[ipoint][ixb] = lFeedDownEfficiency[ipoint][ixb];
                lFeedDownEfficiencyGFCorrectedError[ipoint][ixb] = lFeedDownEfficiencyError[ipoint][ixb];
                } 
           }
        }
        ...
        fLevyFitXiFeedDown = new TF1("LevyFitXiFeedDown",this ,&AliV0Module::MyLevyPtXi, 0.0,15,3, "AliV0Module","MyLevyPXi");

        TFile *fileXiSpectra = new TFile(Form(
            "XiSpectra-%s_ROOTenvelope_0_4.root",fWhichMultEstimator.Data()), "READ"); // xi spectra; errors tot: stat+syst
        TH1F *XiSpectra = (TH1F*) ( fileXiSpectra->Get(Form("hPtXi_%s_0.0_1.0",fWhichMultEstimator.Data()))    );
        TF1 *levyFit = (TF1*)(XiSpectra->GetListOfFunctions()->At(0));
        Double_t lFDParameter_dNdy=levyFit->GetParameter(0);
        Double_t lFDParameter_Temp=levyFit->GetParameter(1);
        Double_t lFDParameter_n=levyFit->GetParameter(2);

        //
        fLevyFitXiFeedDownErr[0] = (TH1D*)fileXiSpectra->Get(Form("HistoFeeddownEnvelopeLow_%s_%1.1f_%1.1f",fWhichMultEstimator.Data(),0.0,1.0)); 
        fLevyFitXiFeedDownErr[1] = (TH1D*)fileXiSpectra->Get(Form("HistoFeeddownEnvelopeHi_%s_%1.1f_%1.1f",fWhichMultEstimator.Data(),0.0,1.0));
        ////// 
