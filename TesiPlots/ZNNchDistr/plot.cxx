...
void plot(Int_t lClassCode = 2)
{
    ...
    for (int i = 0; i < sizePerc; i++)
    {
        pZN[i]->Rebin(10);
        histozn(pZN[i]);
        pZN[i]->GetYaxis()->SetRangeUser(0.00004, pZN[i]->GetMaximum() * 18);
        if (lClassCode == 2)
        {
            pZN[i]->GetYaxis()->SetRangeUser(0.00008, pZN[i]->GetMaximum() * 1.5);
        }
        if (lClassCode == 3)
        {
            pZN[i]->GetYaxis()->SetRangeUser(0.0002, 0.2);
        }
        if (lClassCode == 4)
        {
            pZN[i]->GetYaxis()->SetRangeUser(0.00008, 0.2);
        }
        pZN[i]->SetLineColor(colors[i]);
        pZN[i]->SetMarkerColor(colors[i]);
        pZN[i]->Draw("LEP SAME");
        pZN[i]->Draw("HIST SAME");
    }
    leg5->Draw();
    tex->Draw();
    tex2->Draw();
   ...
   c5->SaveAs(Form("distrnchZN_class%i.pdf",lClassCode));
