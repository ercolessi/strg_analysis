...
void DoUncorrSystExtrap(
    ...
    Double_t lHiEE = 100.){
        ...
        hMaxDevCorr->GetYaxis()->SetRangeUser(0.,0.1);
        ...
        for (int nmult = 0; nmult < multbinnumb; nmult ++){
            ...
            hMaxDev->GetYaxis()->SetRangeUser(0.,0.1);
        }
        ...
        hMaxDev->GetYaxis()->SetRangeUser(0.,.1);
        hMaxDev->SetStats(0);
        hMaxDev->GetYaxis()->SetTitle("Rel. Uncorr. Systematic");
        ...
        hMaxDev->Draw(); 
         if (fWhichEstimator.Contains("V0M")) {xlabel2-> DrawLatex(0.4, 0.81, Form("ZDC fixed [%.0f-%.0f] ",lLoEE,lHiEE));}
        else {xlabel2-> DrawLatex(0.3, 0.81, Form("V0M fixed [%.0f-%.0f]",lLoMult,lHiMult));}
