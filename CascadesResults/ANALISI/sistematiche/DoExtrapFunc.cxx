...
void DoExtrap(
  ...
  void DoExtrapFunc(TString fWhichEstimator = "ZDC", 
  Double_t lLoMult = 0., 
  Double_t lHiMult = 30., 
  Double_t lLoEE = 0., 
  Double_t lHiEE = 100.){
...
void DoExtrap(
  ...
  Double_t lHiEE = 100.) {
    ...
    //
    Double_t percentileV0[] = {0.,1.,5,10,15,20,30,40,50,70,100};
    Double_t systpercentileV0[4] = {0.,10.,30.,100.};
    ...
    for (int i = 0; i < binnumber; i++){ //loop over selection classes   
        ...
        if (Func.Contains("BlastWave")) {
         ...
         // else if(fWhichEstimator.Contains("V0M") &&  (lLoEE == 0. && lHiEE == 30.)) {FFit[i] = PWGFunc->GetBGBW( mass, .8, 0.1, 1., 5.,  Form("%s_%.0f-%.0f","BlastWave",percentile[i],percentile[i+1]));}
          //else {
          if(fWhichEstimator.Contains("ZDC") &&  (lLoMult == 70. && lHiMult == 100.)) {FFit[i] = PWGFunc->GetBGBW( mass, .8, 0.3, 1., 1.,  Form("%s_%.0f-%.0f","BlastWave",percentile[i],percentile[i+1]));}
          else FFit[i] = PWGFunc->GetBGBW( mass, .8, 0.3, 1., 5.,  Form("%s_%.0f-%.0f","BlastWave",percentile[i],percentile[i+1]));//};
        }
    ...
    for (int i = 6; i < 7; i++){ //loop over selection classes 
