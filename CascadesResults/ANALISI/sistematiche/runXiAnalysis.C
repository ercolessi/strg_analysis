...
void runXiAnalysis(
  ...
  //Set data files
  TString lMCFilename = "../MC15g3b1_ZDCRuns.root" ; //0x0
  TString lDataFilename  = "../../15f_nozdc/LHC15f_pass2.root" ;//0x0;
  TString lZDCtoEEFilename  = "../ExtractedZDCPercentile_RD-15f.root" ;//0x0;
  ...
  if( lWhichEstimator.Contains("V0") ) casc->SetOutputFile( Form("Results-%s-13TeV-%s_%03.0f_%03.0f_ZDC_%03.0f_%03.0f.root", lCascType.Data(), lWhichEstimator.Data(), 
    ...
    if (lDoSystematics == kTRUE){
        ...
        cout<<endl<<"---> Performing Systematics Studies: TPC Least Number of Clusters"<<endl;
      	if( lCascType == "XiMinus" || lCascType == "XiPlus" ) {
      	  casc->SetCutLeastNumberOfClusters(75); casc->SetOutputFile( lSystFile + lWhichSystVar + "-1.root" ); casc->DoAnalysis();
      	  casc->SetCutLeastNumberOfClusters(75); casc->SetOutputFile( lSystFile + lWhichSystVar + "-2.root" ); casc->DoAnalysis();
