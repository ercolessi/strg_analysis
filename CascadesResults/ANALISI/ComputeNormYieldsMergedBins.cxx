...
void ComputeNormYieldsMergedBins(
  ...
  Double_t lHiEE = 100.) {
    ...
    Double_t systpercentileV0[4] = {0.,30.,50.,100.};
    Int_t systcounterV0[3] = {6,8,10};
    ...
    for (int i = 0; i < 3; i++){
      ...
      //files
	    fileSyst[i] = TFile::Open(Form("sistematiche/SystematicsFinalResults-Xi-13TeV-V0M_%03.0f_%03.0f_ZDC_%03.0f_%03.0f.root",0.,100.,0.,100.));//lLoMult,lHiMult,lLoEE,lHiEE));//0.,100.,0.,100.));//v0min,v0max,zdcmin,zdcmax));	
      // histo x systematics
    ...
    cout << Form("StatSpectra%s-Xi-%s_%03.0f_%03.0f.root",fWhichEstimator.Data(),fWhichOtherEstimator.Data(),lLo,lHi) << endl;
    ...
    TH1D* hTot_Uncorr = new TH1D("hTot_Uncorr","",50,-.01,0.025);
    ...
    for(int i = 0; i < binnumber ; i++){
      ...
      double c = TMath::Sqrt(hMCVariedUncorr->GetBinContent(i+1)*hMCVariedUncorr->GetBinContent(i+1) + NegSystdNch[i]/dNch[i]*NegSystdNch[i]/dNch[i]);
  ...
  //
   ...
   /* TH1D* g1 = new TH1D("g1", " ", 40, 0., 40.);
    ...
    TFile* Write = new TFile(Form("FinalTestfullfitYield-%s-%s-V0M_%03.0f_%03.0f_ZDC_%03.0f_%03.0f.root","Xi",fWhichEstimator.Data(),lLoMult,lHiMult,lLoEE,lHiEE), "recreate");
