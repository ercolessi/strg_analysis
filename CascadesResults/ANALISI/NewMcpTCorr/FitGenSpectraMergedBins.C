...
//------------------- MAIN FUNCTION ----------------------------
//--------------------------------------------------------------
void FitGenSpectraMergedBins(int iteration = 0, TString fWhichEstimator = "ZDC", Double_t lLoMult = 70., 
Double_t lHiMult = 100., Double_t lLoEE = 0., Double_t lHiEE = 100.) {
    ...
    TFile* fileMC = new TFile("../../CLEAN/MC15g3b1_ZDCRuns.root","READ");
...
//++++++++++++++++++++++++++++++++++++++++ 
 ...
 //  if (fWhichEstimator.Contains("V0M")) hLevyGenerated[9]->SetLineColor(kBlue + 3);
  ...
  if (fWhichEstimator.Contains("V0M")) {
    ...
    l->AddEntry(hLevyGenerated[0],"V0M 0-5% corr. spectrum","LEP");
  ...
  if (fWhichEstimator.Contains("V0M"))xlabe-> DrawLatex(0.78, 0.71,Form("%s fixed [%.0f-%.0f]",fWhichOtherEstimator.Data(),lLoEE,lHiEE));
  else if (fWhichEstimator.Contains("ZDC"))xlabe-> DrawLatex(0.78, 0.71, Form("%s fixed [%.0f-%.0f]",fWhichOtherEstimator.Data(),lLoMult,lHiMult));
...
//++++++++++++++++++++++++++++++++++++++++ 
  ...
  if (fWhichEstimator.Contains("V0M")) {
    l1->AddEntry(hSpectraRatio[0],"V0M 0-5% corr. spectrum","LEP");
    l1->AddEntry(hSpectraRatio[8],"V0M 70-100% corr. spectrum","LEP");
  } else if (fWhichEstimator.Contains("ZDC")) {
    l1->AddEntry(hSpectraRatio[0],"ZDC 0-20% corr. spectrum","LEP");
    if ( lHiMult==100)  l1->AddEntry(hLevyGenerated[7],"ZDC 90-100% corr. spectrum","LEP");
    if ( lHiMult<100)  l1->AddEntry(hLevyGenerated[7],"ZDC 90-100% corr. spectrum","LEP");
  }
  
  l1->Draw("SAME");
  if (fWhichEstimator.Contains("V0M"))xlabe-> DrawLatex(0.32, 0.71, Form("%s fixed [%.0f-%.0f]",fWhichOtherEstimator.Data(),lLoEE,lHiEE));
  else if (fWhichEstimator.Contains("ZDC"))xlabe-> DrawLatex(0.32, 0.71, Form("%s fixed [%.0f-%.0f]",fWhichOtherEstimator.Data(),lLoMult,lHiMult));
