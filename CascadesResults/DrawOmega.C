...
void DrawOmega(){
	...
	TF1 * LevyTsallis(const Char_t *name, Double_t mass, Double_t n = 5., Double_t C = 0.1, Double_t norm = 1.);
	const int nbins = 6;
	Double_t centrality[nbins] = {0,5,15,30,50,100};
	...
	TH1F* hsgnloss[10];
	for (int i =0; i<10;i++){
	    hsgnloss[i] = (TH1F*)file->Get(Form("SgnLoss/%s/%s/fHistptSel%s_%.0f-%.0f_%s",Type.Data(),"multsel", "V0",centrality[i],centrality[i+1],Type.Data()));
	}
	double evtloss15[5];
	...
	for (int i = 1; i < nbins; i++){
	     filename[i] = new TFile(Form("15f/Results-%s-13TeV-V0M_%03.0f_%03.0f_ZDC_000_100.root",Type.Data(),centrality[i-1],centrality[i]),"READ");
	...
	s->Draw();
	s->SaveAs(Form("%s%sV0Omega.png",Type.Data(),period.Data())); 
