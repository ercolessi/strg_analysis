...
void runCascadeAnalysis(
  ...
  //Compile Macro
  Int_t workedornot = 1;// gSystem->CompileMacro("AliCascadeModule.cxx","-kfo");
  cout<<"----------------------------------------------------"<<endl;
  cout<<endl;
  if( workedornot == 0 ){ 
      cout<<"*************************************"<<endl;
  ...
  //
  // signal extraction
  casc->SetCutSigmaForSignalExtraction                     ( 6.0 ); // 4.0 );
  //-----------------------------------------------------------------------------------------------
  ...
  /*if( lCascType.Contains("Plus") ) {
      TFile* fGeantFluka = new TFile("/home/francesca/Strangeness/Current/Systematics/aCorrectionForPPbarCrossSections_FromPanos.root", "READ");
  ...
  //Set data files
  TString lMCFilename = "/alistorage/effectiveenergy/FilesTreni_INELgtZERO/MCLHC15g3b1_TrainMerge.root" ; //0x0
  TString lDataFilename  = "/alistorage/effectiveenergy/FilesTreni_INELgtZERO/LHC15f_TrainMerge.root" ;//0x0;
  //TString lZDCtoEEFilename  = "ExtractedZDCPercentile_15h.root" ;//0x0;
  // 
  casc->SetRealDataFile( lDataFilename.Data() );
  casc->SetMCDataFile ( lMCFilename.Data()   );
  //casc->SetZDCtoEEFileName ( lZDCtoEEFilename.Data()  );
