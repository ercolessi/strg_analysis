void preppadup(TPad *pad1, TPad *pad2, TPad *pad3);
void preppaddown(TPad *pad1, TPad *pad2, TPad *pad3);
...
void PlotFinal(bool donch = 0)
{
    ...
    prepcanvas(c);
    TPad *pad1 = new TPad("pad1", "pad1", 0., 0., 0.4, 0.53);
    TPad *pad2 = new TPad("pad2", "pad2", 0.4, 0, 0.7, 0.53);
    TPad *pad3 = new TPad("pad3", "pad3", 0.7, 0, 1., 0.53);
    preppaddown(pad1, pad2, pad3);

    TPad *pad1up = new TPad("pad1up", "pad1up", 0., 0.53, 0.4, 1.);
    TPad *pad2up = new TPad("pad2up", "pad2up", 0.4, 0.53, 0.7, 1.);
    TPad *pad3up = new TPad("pad3up", "pad3up", 0.7, 0.53, 1., 1.);
    preppadup(pad1up, pad2up, pad3up);
    ...
    l->DrawLatex(0.85, 0.75, Form("%s", "|#it{y}|<0.5"));
    l2->DrawLatex(0.47, 0.85, Form("%s", "ALICE, pp #sqrt{s} = 13 TeV"));
    lbig->DrawLatex(0.85, 0.85, Form("%s", "K^{0}_{S}"));
    ...
    l2->DrawLatex(0.85, 0.75, Form("%s", "|#it{y}|<0.5"));
    lbig2->DrawLatex(0.85, 0.85, Form("%s", "#Lambda"));
    ...
    l2->DrawLatex(0.85, 0.75, Form("%s", "|#it{y}|<0.5"));
    lbig2->DrawLatex(0.85, 0.85, Form("%s", "#Xi"));
    ...
    TLegend *leg2 = new TLegend(0.3, 0.72, 0.5, 0.92);
    ...
    l2->DrawLatex(0.85, 0.82, Form("%s", "|#it{y}|<0.5"));
    lbig2->DrawLatex(0.85, 0.91, Form("%s", "#Xi"));

    c->SaveAs(Form("images/FinalPlot%s.pdf", donch ? "Nch" : "ZDCSum"));
...
void prephisto(TH1D* h = 0x0, bool donch = 1){
    ...
    h->GetYaxis()->SetRangeUser(0.45, 1.62);
    h->SetTitle("");
