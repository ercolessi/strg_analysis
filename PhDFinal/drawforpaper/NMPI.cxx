...
{ //var = "AvPt"
    TFile *file = TFile::Open(Form("../models/Results_%s_SPDClustersV0M.root",pythia.Data()));
    ...
    for (int i = 0; i < nsel; i++)
    {
        if (var.Contains("NMPI")){
            gNMPILE[i] = (TGraphErrors *)file->Get(Form("%s/NormLEVsNMPI", ddname[i].Data()));
            gNMPINch[i] = (TGraphErrors *)file->Get(Form("%s/NormNchVsNMPI", ddname[i].Data()));
        } else if (var.Contains("AvPt")){
            gNMPILE[i] = (TGraphErrors *)file->Get(Form("%s/MeanPt/AvPt_VsLeadENorm_PiMinus", ddname[i].Data()));
            gNMPINch[i] = (TGraphErrors *)file->Get(Form("%s/MeanPt/AvPt_VsNchNorm_PiMinus", ddname[i].Data()));
        }
    }

    if (pythia.Contains("Monash")){
        prepgraph(gNMPILE[0], kOpenDiamond, 2.3, kBlack);
        prepgraph(gNMPILE[1], kOpenCircle, 1.8, kRed);
        prepgraph(gNMPILE[2], kOpenCircle, 1.8, kBlue);
        prepgraph(gNMPILE[3], kOpenSquare, 1.8, kViolet);
        prepgraph(gNMPILE[4], kOpenSquare, 1.8, kGreen + 1);

        prepgraph(gNMPINch[0], kOpenDiamond, 2.3, kBlack);
        prepgraph(gNMPINch[1], kOpenCircle, 1.8, kRed);
        prepgraph(gNMPINch[2], kOpenCircle, 1.8, kBlue);
        prepgraph(gNMPINch[3], kOpenSquare, 1.8, kViolet);
        prepgraph(gNMPINch[4], kOpenSquare, 1.8, kGreen + 1);
    ...
    TH1F *hnch = new TH1F("h", "", 100, 0, 4.4);
    hnch->SetStats(0);
    hnch->GetYaxis()->SetTitle("#LT N_{MPI} #GT");
    if (var.Contains("AvPt")) hnch->GetYaxis()->SetTitle("#LT p_{T} #GT (#pi^{-})");
    hnch->GetXaxis()->SetTitle(" n_{ch} / #LT n_{ch} #GT_{INEL>0} (|#eta|<0.5)");
    ...
    if (var.Contains("NMPI")){
        ...
        if (pythia.Contains("Ropes")) hnch->GetYaxis()->SetRangeUser(0.35, 0.65);
        if (pythia.Contains("Monash")) hnch->GetYaxis()->SetRangeUser(0.35, 0.75);
    }


    TH1F *hzdc = new TH1F("hzdc", "", 100, 0, 1.4);
    hzdc->SetStats(0);
    hzdc->GetYaxis()->SetTitle("#LT N_{MPI} #GT");
    hzdc->GetXaxis()->SetTitle("E^{ZN}_{leading} / #LT E^{ZN}_{leading} #GT_{INEL>0} (|#eta|>8)");
    ...
    if (var.Contains("NMPI")){
        ...
        if (pythia.Contains("Ropes")) hzdc->GetYaxis()->SetRangeUser(0.35, 0.65);
        if (pythia.Contains("Monash")) hzdc->GetYaxis()->SetRangeUser(0.35, 0.75);
    }
