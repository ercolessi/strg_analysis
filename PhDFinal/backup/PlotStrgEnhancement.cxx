...
void PlotStrgEnhancement(int nclass = 5,TString num = "Omega", TString den = "nch")
{

    TString sel[] = {"_SPDClustersV0M_class10", Form("_SPDClustersV0M_class%i", nclass)};
    ...
    prepgraph(gNchRatioStat[1], kFullCircle, 2.5, kBlack);
    prepgraph(gZDCRatioStat[1], kFullCircle, 2.5, kBlack);
    ...
    if (num.Contains("Omega") && den.Contains("nch"))
        hnch->GetYaxis()->SetTitle("#frac{#Omega}{n_{ch}}");
    ...
    Int_t colors[] = {kRed + 1, kOrange+1, kYellow+1, kSpring - 1, kAzure + 7, kBlue + 2};
