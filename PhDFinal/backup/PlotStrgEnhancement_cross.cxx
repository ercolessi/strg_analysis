...
void PlotStrgEnhancement_cross(int ishighmult = 1, TString num = "K0Short", TString den = "nch", bool domodel = 1, TString pythia = "PythiaRopes")
{
    ...
    prepgraph(gNchRatioStat[0], kFullDiamond, 2.8, kBlack);
    prepgraph(gZDCRatioStat[0], kFullDiamond, 2.8, kBlack);
    ...
    if (num.Contains("K0Short")) num_ = "K0Short";
    ...
    if (domodel){
        ...
        file1 = TFile::Open(Form("models/PostProcessRatio%s%s_%s.root", num_.Data(), den_.Data(), pythia.Data()));
        ...
        file2 = TFile::Open(Form("models/PostProcessRatio%s%s_%s.root", num_.Data(), den_.Data(), "PythiaMonash"));
        ...
        prepgraph(gnchclass0, 0, 0, kGray + 2, 2, 1, 0);
        prepgraph(gnchclass1, 0, 0, kGray + 2, 2, 7, 0);
        prepgraph(gnchclass2, 0, 0, kGray + 2, 2, 3, 0);
        prepgraph(gzdcclass0, 0, 0, kGray + 2, 2, 1, 0);
        prepgraph(gzdcclass1, 0, 0, kGray + 2, 2, 7, 0);
        prepgraph(gzdcclass2, 0, 0, kGray + 2, 2, 3, 0);

        prepgraph(bisgnchclass0, 0, 0, kMagenta-3, 2, 1, 0);
        prepgraph(bisgnchclass1, 0, 0, kMagenta-3, 2, 7, 0);
        prepgraph(bisgnchclass2, 0, 0, kMagenta-3, 2, 3, 0);
        prepgraph(bisgzdcclass0, 0, 0, kMagenta-3, 2, 1, 0);
        prepgraph(bisgzdcclass1, 0, 0, kMagenta-3, 2, 7, 0);
        prepgraph(bisgzdcclass2, 0, 0, kMagenta-3, 2, 3, 0);
  ...
  //  Int_t colors2[] = {kRed, kRed, kRed, kRed, kRed, kRed, kRed, kRed};
   // Int_t colors3[] = {kRed, kRed, kRed, kRed, kRed, kRed, kRed, kRed};
    ...
    lXi1->AddEntry(marker1, "SPDcl+V0M, fixed #LT n_{ch} #GT", "P");
    lXi1->AddEntry(marker2, "SPDcl+V0M, fixed #LT ZN #GT", "P");
    ...
    TLegend *lmodel = new TLegend(0.03, 0.18, 0.32, 0.4);
    if (domodel){
        lmodel->SetBorderSize(0);
        lmodel->AddEntry(bisgnchclass0, "Pythia8 Monash", "L");
        lmodel->AddEntry(gnchclass0, "Pythia8 + Ropes", "L");
        lmodel->AddEntry(legendgnchclass0, "V0M standalone", "L");
        lmodel->AddEntry(legendgnchclass1, "SPDcl+V0M, fixed #LT n_{ch} #GT", "L");
        lmodel->AddEntry(legendgnchclass2, "SPDcl+V0M, fixed #LT E_lead #GT", "L");
