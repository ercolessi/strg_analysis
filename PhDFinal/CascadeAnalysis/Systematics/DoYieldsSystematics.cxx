...
TH1 *YieldMean(TH1 *hstat, TF1 *f = NULL, Double_t minfit = 0.6, Double_t maxfit = 6.5, TString logfilename = "log.root", Double_t min = 0.0, Double_t max = 10., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q");
    ...
    Bool_t kDoMB = kFALSE)
{
  ...
  if (kDoMB) outputfilename = Form("YieldSystematics-%s-13TeV-INELgt0.root", fWhichParticle.Data());
    ...
    Int_t nbin = 0)
{
  ...
  // Fit function
  TF1 *LevyTsallisfunc[binnumber][5];
  ...
  { // loop over selection classes
    ...
    if (!lCascType.Contains("K0Short"))
      lAntiResultsFile[i][0] = TFile::Open(Form("files/%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f.root", fWhichAntiParticle.Data(), fWhichAntiParticle.Data(), spdmin, spdmax, v0min, v0max));
  ...
  { // loop over selection classes
    for (int j = 0; j < 5; j++)
    {
      LevyTsallisfunc[i][j] = LevyTsallis(Form("LevyTsallisfunc%i%i", i, j), mass);

      hout[i][j] = (TH1F *)YieldMean(lHistPtClone[i][j], LevyTsallisfunc[i][j], minfit, maxfit, Form("log%s-%s_%03.0f_%03.0f.root", lCascType.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi));
      // Get yield
  ...
  for (int i = 1; i <= hYield[0]->GetNbinsX(); i++)
  {
    for (int k = 1; k < 5; k++)
    {
      ...
      if (PassRogerBarlowCriterion(1, hYield[k], i))
      {
        binvalue[k - 1] = TMath::Abs(hYield[k]->GetBinContent(i) - 1);
      }
    }
    maxvalue = binvalue[0];
    int counter = 1;
  ...
  hMaxDev->GetXaxis()->SetTitleOffset(1.2);
  ...
  maxdev->SaveAs(Form("images/yields/fixed%s/YieldsMaxDev-%s-%s-%s%.0f%0.f.png", fWhichFixedEstimator.Data(), lCascType.Data(), lWhichSystVar.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi));
  ...
  hYield[1]->GetXaxis()->SetTitleOffset(1.1);
  ...
  legend->Draw("SAME");
  labbig->DrawLatex(0.35, 0.76, Form("%s [%.0f-%0.f]%s", fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "%"));
  cutvar->SaveAs(Form("images/yields/fixed%s/YieldsCutVar-%s-%s-%s%.0f%0.f.png", fWhichFixedEstimator.Data(), lCascType.Data(), lWhichSystVar.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi));
...
/**/
//--------------------------------------------------------------------------------------------
TH1 *YieldMean(TH1 *hstat, TF1 *f = NULL, Double_t minfit = 0.6, Double_t maxfit = 6.5, TString logfilename = "log.root", Double_t min = 0., Double_t max = 10., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q")
{
  if (maxfit > max)
    max = maxfit;
  if (minfit < min)
...
Bool_t PassRogerBarlowCriterion(int nsigmas, TH1F *h, int bin)
{
  ...
  if (TMath::Abs(dev - 1) > (nsigmas * RBsigma))
  {
    return kTRUE;
  }
  else
