    ...
    Bool_t kDoMB = kFALSE)
{
  ...
  Float_t *percentile;
  Long_t percbinnumb;
  Float_t p1[] = {0, 10, 20, 30, 40, 50, 70, 100}; // 7
  Long_t n1 = sizeof(p1) / sizeof(Float_t) - 1;
  Float_t pmb[] = {0, 100}; // 7
  Long_t nmb = sizeof(pmb) / sizeof(Float_t) - 1;
  Float_t p2[] = {0, 20, 40, 100}; // 7
  Long_t n2 = sizeof(p2) / sizeof(Float_t) - 1;
  Float_t p3[] = {0, 20, 60, 100}; // 7
  Long_t n3 = sizeof(p3) / sizeof(Float_t) - 1;
  ...
  TString outputfilename = Form("YieldSystematics-%s-13TeV-%s_%03.0f_%03.0f.root", fWhichParticle.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi);
  if (kDoMB)
    outputfilename = Form("YieldSystematics-%s-13TeV-INELgt0.root", fWhichParticle.Data());

  // Selection classes
  TH1F *hV0Radius = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "V0Radius", percentile, percbinnumb);
  TH1F *hCascRadius = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "CascRadius", percentile, percbinnumb);
  TH1F *hNegToPV = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "DCANegToPV", percentile, percbinnumb);
  TH1F *hPosToPV = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "DCAPosToPV", percentile, percbinnumb);
  TH1F *hV0ToPV = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "DCAV0ToPV", percentile, percbinnumb);
  TH1F *hBachToPV = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "DCABachToPV", percentile, percbinnumb);
  TH1F *hV0Daught = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "DCAV0Daughters", percentile, percbinnumb);
  TH1F *hCascDaught = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "DCACascDaughters", percentile, percbinnumb);
  TH1F *hV0CosPA = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "V0CosPA", percentile, percbinnumb);
  TH1F *hCascCosPA = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "CascCosPA", percentile, percbinnumb);
  TH1F *hV0Mass = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "V0Mass", percentile, percbinnumb);
  TH1F *hCompetingSpecies = 0x0;
  TH1F *hPLT = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "ProperLifetime", percentile, percbinnumb);
  TH1F *hTPCNSigmas = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "TPCPIDNSigmas", percentile, percbinnumb);
  TH1F *hTPCNClusters = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "TPCNClusters", percentile, percbinnumb);
  TH1F *hSigExtBinCount = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "SigExtBinCount", percentile, percbinnumb);
...
TH1F* ComputeYieldsforSystematics(
    ...
    TFile *fileSyst = TFile::Open(Form("../../correctedspectra/CorrSpectra-%s-13TeV_INELgt0.root", lCascType.Data()));
    TH1F *hTotalSyst = (TH1F *)fileSyst->Get("FinalSpectra/hTotalSyst");
    for (int i = 0; i < binnumber; i++){ //loop over selection classes
        ...
        //
        TString lSystFile = Form("files/%s/Results-Systematics", fWhichParticle.Data());
        lSystFile.Append( Form( "-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f-", fWhichParticle.Data(), spdmin, spdmax, v0min, v0max) );
        TString lAntiSystFile = Form("files/%s/Results-Systematics", fWhichAntiParticle.Data());
        ...
        //
        lResultsFile[i][0] = TFile::Open(Form("files/%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f.root", fWhichParticle.Data(), fWhichParticle.Data(), spdmin, spdmax, v0min, v0max));
        if (!lCascType.Contains("K0Short")) lAntiResultsFile[i][0] = TFile::Open(Form("files/%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f.root", fWhichAntiParticle.Data(), fWhichAntiParticle.Data(), spdmin, spdmax, v0min, v0max));
        ...
        if (!lCascType.Contains("K0Short")) lAntiHistPt[i][0] = (TH1F*)lAntiResultsFile[i][0]->Get(Form("fHistPt%s", fWhichAntiParticle.Data()));

        // Add Spectra
        for (int j = 0; j < 5; j++){
          lHistPtClone[i][j] = (TH1F*)lHistPt[i][j]->Clone(Form("lHistPt%i%i",i,j));
          lSystPt[i][j] = (TH1F *)lHistPt[i][j]->Clone(Form("lSystPt%i%i", i, j));
          if (!lCascType.Contains("K0Short")) {
            ...
            for (int bin = 1; bin <=  lHistPt[i][j]->GetNbinsX(); bin ++){
              ...
              lSystPt[i][j]->SetBinContent(bin, lHistPt[i][j]->GetBinContent(bin) + lAntiHistPt[i][j]->GetBinContent(bin));
              lSystPt[i][j]->SetBinError(bin, lSystPt[i][j]->GetBinContent(bin) * hTotalSyst->GetBinContent(bin));
            }
          }
        }
    }
    ...
    // Get the yields
      for (int i = 0; i < binnumber; i++){ //loop over selection classes
      for (int j = 0; j < 5; j++){
          hout[i][j] = (TH1F *)YieldMean(lHistPtClone[i][j], lSystPt[i][j], LevyTsallisfunc, minfit, maxfit, Form("log%s-%s_%03.0f_%03.0f.root", fWhichParticle.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi));
          // Get yield
    ...
    hMaxDev->Draw();
    labbig->DrawLatex(0.35, 0.76, Form("%s [%.0f-%0.f]%s",fWhichFixedEstimator.Data(),lFixedLo,lFixedHi,"%"));
    maxdev->SaveAs(Form("images/yields/fixed%s/YieldsMaxDev-%s-%s-%s%.0f%0.f.png", fWhichFixedEstimator.Data(), fWhichParticle.Data(), lWhichSystVar.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi));
    ...
    for (int k = 1; k < 5; k++){
      ...
      hYield[k]->GetYaxis()->SetRangeUser(1 - hMaxDev->GetMaximum() * 2., 1 + hMaxDev->GetMaximum() * 2.);
    ...
    legend->Draw("SAME");
    labbig->DrawLatex(0.35, 0.76, Form("%s [%.0f-%0.f]%s",fWhichFixedEstimator.Data(),lFixedLo,lFixedHi,"%"));
    cutvar->SaveAs(Form("images/yields/fixed%s/YieldsCutVar-%s-%s-%s%.0f%0.f.png", fWhichFixedEstimator.Data(), fWhichParticle.Data(), lWhichSystVar.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi));

    // Write on file, update
    TFile* lOutputFile = TFile::Open(filename, "UPDATE");
    TDirectoryFile *lDirCutVar = new TDirectoryFile(Form("%sVar%s",fWhichVarEstimator.Data(),lWhichSystVar.Data()),Form("Variation Cuts %s",lWhichSystVar.Data()));
    lDirCutVar->cd();
    for (int k = 1; k < 5; k++){
      hYield[k]->Write();
    }
...
TH1 *YieldMean(TH1 *hstat, TH1 *hsys, TF1 *f = NULL, Double_t minfit = .4, Double_t maxfit = 8., TString logfilename = "log.root", Double_t min = 0., Double_t max = 100., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q")
{
    if (maxfit > max)
      max = maxfit;
    if (minfit < min)
