...
void AnalyseMC(TString mcname = "Ropes")
{
...
void process(TString directory = "", TString outputfile = "", TString option = "RECREATE", TString mcname = "EPOSLHC", Double_t *SPDClustersperclow = 0x0, Double_t *SPDClustersperchigh = 0x0, Double_t *V0Mperclow = 0x0, Double_t *V0Mperchigh = 0x0, Int_t n = 0)
{
    ...
    //ZN must be taken from another file
    TFile* fileZN;
    if (mcname.Contains("Monash"))
        fileZN = TFile::Open(Form("Files/%s.root", "PythiaMonash_FN_ZDCN_eta8"));
    else if (mcname.Contains("Ropes")){
        fileZN = TFile::Open(Form("Files/%s.root", "PythiaRopes_FN_ZDCN_eta8"));
    }
    fileZN->cd("PWGLF_MCPredictionsStrgVsMultVsZDC");
    TList *listZN = (TList *)fileZN->FindObjectAny("cList");
    ...
    TH1D *fHistV0MMult_ZN;
    TH1D *fHistSPDClusters_ZN;
    ...
    //ZN must be taken from another file
    f2DHistLeadingESPDV0M = (TH2D *)listZN->FindObject("f2DHistLeadingESPDV0M");
    f2DHistINELgt0SPDV0M_ZN = (TH2D *)listZN->FindObject("f2DHistINELgt0SPDV0M");
    fHistV0MMult_ZN = (TH1D *)listZN->FindObject("fHistV0MMult");
    fHistSPDClusters_ZN = (TH1D *)listZN->FindObject("fHistSPDClusters");

    // Percentile calibrations
    TH1D *hcalibV0M_ZN = dopercentile(fHistV0MMult_ZN);
    hcalibV0M_ZN->SetName("hcalibV0M_ZN");
    TH1D *hcalibSPDCl_ZN = dopercentile(fHistSPDClusters_ZN);
    hcalibSPDCl_ZN->SetName("hcalibSPDCl_ZN");

    // Convert to values V0M and SPDCl
    Double_t V0Mvallow_ZN[nDiff], V0Mvalhigh_ZN[nDiff];
    Double_t SPDClvallow_ZN[nDiff], SPDClvalhigh_ZN[nDiff];
    for(int i = 0; i < n; i++){
        V0Mvallow_ZN[i] = getval(hcalibV0M_ZN, V0Mperclow[i]);
        V0Mvalhigh_ZN[i] = getval(hcalibV0M_ZN, V0Mperchigh[i]);
        SPDClvallow_ZN[i] = getval(hcalibSPDCl_ZN, SPDClustersperclow[i]);
        SPDClvalhigh_ZN[i] = getval(hcalibSPDCl_ZN, SPDClustersperchigh[i]);
    }


    //Variable definition
    Double_t EvtMB, EvtMB_e, EvtMB_ZN, EvtMB_ZN_e; // errors must be double for the method TH1::IntegralAndError()
    ...
    Double_t Evt[nDiff], Evt_e[nDiff], Evt_ZN[nDiff], Evt_ZN_e[nDiff];
    ...
    //
    EvtMB_ZN = f2DHistINELgt0SPDV0M_ZN->IntegralAndError(
        1, f2DHistINELgt0SPDV0M_ZN->GetNbinsX(),
        1, f2DHistINELgt0SPDV0M_ZN->GetNbinsY(),
        EvtMB_ZN_e);
    //
    ...
    doerrorAB(LeadingEMB, LeadingEMB_e, EvtMB_ZN, EvtMB_ZN_e);
    LeadingEMB /= EvtMB_ZN;
    //
    if (isPythia){
    ...
    for(int k = 0; k < nDiff; k++){ // SPD Cluster loop
            ...
            //
            Evt_ZN[k] = f2DHistINELgt0SPDV0M_ZN->IntegralAndError(
                f2DHistINELgt0SPDV0M_ZN->GetXaxis()->FindBin(SPDClvalhigh_ZN[k]), f2DHistINELgt0SPDV0M_ZN->GetXaxis()->FindBin(SPDClvallow_ZN[k]),
                f2DHistINELgt0SPDV0M_ZN->GetYaxis()->FindBin(V0Mvalhigh_ZN[k]), f2DHistINELgt0SPDV0M_ZN->GetYaxis()->FindBin(V0Mvallow_ZN[k]),
                Evt_ZN_e[k]);
            //
            ...
            LeadingENorm[k] = f2DHistLeadingESPDV0M -> IntegralAndError(
                f2DHistLeadingESPDV0M->GetXaxis()->FindBin(SPDClvalhigh_ZN[k]), f2DHistLeadingESPDV0M->GetXaxis()->FindBin(SPDClvallow_ZN[k]),
                f2DHistLeadingESPDV0M->GetYaxis()->FindBin(V0Mvalhigh_ZN[k]), f2DHistLeadingESPDV0M->GetYaxis()->FindBin(V0Mvallow_ZN[k]),
                LeadingENorm_e[k]
            );
            LeadingE_e[k] = LeadingENorm_e[k];
            doerrorAB(LeadingENorm[k], LeadingE_e[k], Evt_ZN[k], Evt_ZN_e[k]);
            doerrorABC(LeadingENorm[k], LeadingENorm_e[k], Evt_ZN[k], Evt_ZN_e[k], LeadingEMB, LeadingEMB_e);
            LeadingENorm[k] /= Evt_ZN[k];
    ...
    if (isPythia) {
            gNchVsNMPI = new TGraphErrors(nDiff, NchNorm, NMPI, NchNorm_e, NMPI_e);
            gNchVsNMPI->GetYaxis()->SetTitle("NMPI");
            gNchVsNMPI->GetXaxis()->SetTitle("Nch/Nch_{MB}");
            gNchVsNMPI->SetTitle("");
            gNchVsNMPI->SetMarkerStyle(kFullCircle);
            gNchVsNMPI->SetName("NormNchVsNMPI");
            //
            gLEVsNMPI = new TGraphErrors(nDiff, LeadingENorm, NMPI, LeadingENorm_e, NMPI_e);
            gLEVsNMPI->GetYaxis()->SetTitle("NMPI");
            gLEVsNMPI->GetXaxis()->SetTitle("Leading Energy / Leading Energy_{MB}");
            gLEVsNMPI->SetTitle("");
            gLEVsNMPI->SetMarkerStyle(kFullCircle);
            gLEVsNMPI->SetName("NormLEVsNMPI");
            //
