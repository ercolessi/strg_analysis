    ...
    Bool_t DoMB = kFALSE)
{
  ...
  else
  {
    ...
    if (DoMB) {
      inputfilename = Form("../../correctedspectra/CorrSpectra-%s-13TeV_INELgt0.root",lCascType.Data());
    } else {
      inputfilename = Form("../../correctedspectra/CorrSpectra-%s-13TeV_%s_Fixed%sin%03.0f_%03.0f.root",
    ...
    if (0){
    DoExtrap( "Levy", inputfilename, percentile, percbinnumb, outputfilename, lCascType, lWhichVarEstimator ,lFixedLo,lFixedHi, DoMB  );
    DoExtrap( "BlastWave", inputfilename, percentile, percbinnumb, outputfilename, lCascType, lWhichVarEstimator ,lFixedLo,lFixedHi, DoMB );
    DoExtrap( "Boltz", inputfilename, percentile, percbinnumb, outputfilename, lCascType, lWhichVarEstimator ,lFixedLo,lFixedHi, DoMB );
    DoExtrap( "MTexpo", inputfilename, percentile, percbinnumb, outputfilename, lCascType, lWhichVarEstimator ,lFixedLo,lFixedHi, DoMB );
    DoExtrap( "FermiDirac", inputfilename, percentile, percbinnumb, outputfilename, lCascType, lWhichVarEstimator ,lFixedLo,lFixedHi, DoMB );
    DoExtrap( "BoseEinstein", inputfilename, percentile, percbinnumb, outputfilename, lCascType, lWhichVarEstimator ,lFixedLo,lFixedHi, DoMB );
    } else {
    DoExtrap("BoseEinstein", inputfilename, percentile, percbinnumb, outputfilename, lCascType, lWhichVarEstimator, lFixedLo, lFixedHi, DoMB);
    }
}
      ...
      Bool_t DoMB = kFALSE)
  {
    ...
    for (int i = 0; i < binnumber; i++){
      ...
      lHistPt[i] = (TH1D *)lResultsFile->Get(Form("FinalSpectra/PtSpectrumCorrStat_%s_%.0f_%.0f_%s_%.0f_%.0f", "SPDClusters", multmin, multmax, "V0M", eemin, eemax));
      ...
      lSystPt[i] = (TH1D *)lResultsFile->Get(Form("FinalSpectra/PtSpectrumCorrSystTot_%s_%.0f_%.0f_%s_%.0f_%.0f", "SPDClusters", multmin, multmax, "V0M", eemin, eemax));
    ...
    if (lCascType.Contains("Lambda")){
      if (Func.Contains("Levy")){
        if (fWhichEstimator.Contains("V0M") && lFixedLo == 0.) {
          if (DoMB){
        ...
        if (fWhichEstimator.Contains("SPDClusters") && lFixedLo == 10.) {};
        if (fWhichEstimator.Contains("SPDClusters") && lFixedLo == 40.) {};
        if (fWhichEstimator.Contains("V0M") && lFixedLo == 10.) {};
        if (fWhichEstimator.Contains("V0M") && lFixedLo == 10.) {};
      }
      ...
      if (Func.Contains("MTexpo")){
        if (fWhichEstimator.Contains("V0M") && lFixedLo == 0.) {if (DoMB){
            max[0] = 1.5;
          } else {
            max[0] = 4;
            max[1] = 3;
    ...
    lHistPtClone[0]->GetXaxis()->SetRangeUser(0.,8.);
    ...
    TFile* Write = new TFile(outputfile, option.Data());
