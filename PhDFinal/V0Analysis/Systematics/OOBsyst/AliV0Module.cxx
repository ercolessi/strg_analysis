...
void AliV0Module::DoAnalysis() {
    ...
    ********************************************************/
    ...
    for(Long_t icand = 0; icand<lNCandidates; icand++) {
        ...
        if(TMath::Abs(lRap)<fRapidityBoundary &&
                ...
                CheckITSTOFOne( kTRUE, lPt, 10., lPosTrackStatus,  lNegTrackStatus,  fTreeVariablePosTOFBCid,  fTreeVariableNegTOFBCid)
   ...
   // === for systematics due to Xi spectra
    ...
    fLevyFit = (TF1*)(fileXiSpectra->Get(Form("NormCorr/AfterSignalLossCorr/LevyFit%s_%.0f_%.0f_%s_%.0f_%.0f","SPDClusters", fLoMultBound, fHiMultBound, "V0M", fLoEEBound, fHiEEBound)));

    Double_t lFDParameter_dNdy=fLevyFit->GetParameter("norm");
    Double_t lFDParameter_Temp=fLevyFit->GetParameter("C");
    Double_t lFDParameter_n=fLevyFit->GetParameter("n");
