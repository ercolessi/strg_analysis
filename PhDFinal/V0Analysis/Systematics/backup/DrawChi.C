void DrawChi( 
  ...
  Double_t lFixedHi = 50.){
    ...
    ChiF[0]->GetYaxis()->SetRangeUser(0.,8.);
    ...
    h[0]->GetYaxis()->SetRangeUser(1E-6,1E3);
    h[0]->GetYaxis()->SetTitle("1/N_{ev} d^{2}N/(dp_{T}dy) [(GeV/c)^{-1}]");
    h[0]->GetXaxis()->SetRangeUser(0.,8.);
    h[0]->SetTitle("");
    h[0]->Draw();
    ...
    xlabel2-> DrawLatex(0.4, 0.81, Form("%s fixed [%.0f-%.0f] ",sel.Data(),lFixedLo, lFixedHi));
    xlabel3-> DrawLatex(0.4, 0.76, "#Lambda + #bar{#Lambda}");
    lf->Draw("SAME");
