...
TH1 *YieldMean(TH1 *hstat, TF1 *f = NULL, Double_t min = 0., Double_t max = 100., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q",TString logfilename="log.root",Double_t minfit=0.6,Double_t maxfit=6.5);
...
void DoYieldsSystematics(
  ...
  Bool_t kDoMB = kFALSE  ){
    ...
    Float_t * percentile;
    Long_t percbinnumb;
    Float_t p1[] = {0,5,10,15,20,30,40,50,70,100}; //7
    ...
    TString outputfilename = Form("SystematicsYields-%s-13TeV-%s_%03.0f_%03.0f.root",fWhichParticle.Data(),fWhichFixedEstimator.Data(),lFixedLo, lFixedHi);
...
TH1F* ComputeYieldsforSystematics( 
    ...
    for (int i = 0; i < binnumber; i++){ //loop over selection classes           
        ...
        TString lAntiSystFile = "files/Results-Systematics";
        ...
        //
        lResultsFile[i][0] = TFile::Open(Form( "files/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_FDUseMCRatio.root", fWhichParticle.Data(), spdmin, spdmax, v0min, v0max));
        lAntiResultsFile[i][0] = TFile::Open(Form( "files/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_FDUseMCRatio.root", fWhichAntiParticle.Data(), spdmin, spdmax, v0min, v0max));
    ...
    // Get the yields
      for (int i = 0; i < binnumber; i++){ //loop over selection classes  
      for (int j = 0; j < 5; j++){
        hout[i][j] = (TH1F*)YieldMean(lHistPtClone[i][j],LevyTsallisfunc);
        // Get yield
    ...
    hMaxDev->GetYaxis()->SetRangeUser(-0.0005,.005);
    ...
    maxdev->SaveAs(Form("images/systyields/fixed%s/Yields-%s-MaxRelDev%s-%s.png",fWhichFixedEstimator.Data(),fWhichParticle.Data(),lWhichSystVar.Data(),fWhichFixedEstimator.Data()));
    ...
    legend->Draw("SAME");    
    labbig->DrawLatex(0.35, 0.76, Form("%s [%.0f-%0.f]%s",fWhichFixedEstimator.Data(),lFixedLo,lFixedHi,"%"));
    cutvar->SaveAs(Form("images/systyields/fixed%s/Yields-%s-CutVar-%s-%s.png",fWhichFixedEstimator.Data(),fWhichParticle.Data(),lWhichSystVar.Data(),fWhichFixedEstimator.Data()));
