  ...
  Bool_t DoMult = kTRUE)
{
  ...
  if (Type.Contains("Lambda")) TH1F *hCompetingSpecies = makeSystPlotsV0s(outputfilename, Type, LowMult, HighMult, LowEE, HighEE, "Competingrejection");
  ...
  hNCr->Draw("HIST SAME");
  l->Draw("SAME");
  if () xlabel-> DrawLatex(0.25, 0.8,"#Lambda + #bar{#Lambda}");
  Top->SaveAs(Form("imagesoff/TopologicalSystematics%s_V0-%03.0f_%03.0f-V0M-%03.0f_%03.0f.png",Type.Data(),LowMult, HighMult, LowEE, HighEE));
  ...
  for (int i = 1; i<= hV0Radius->GetNbinsX(); i++){
    ...
    Double_t CompetingSpecies = 0; //FIXME
    if (Type.Contains("Lambda"))
      hCompetingSpecies->GetBinContent(i);
  ...
  hPLT->Draw("HIST ");
  if (Type.Contains("Lambda")) // FIXMEhCompetingSpecies->Draw("HIST SAME ");
    hTPCNClusters->Draw("HIST SAME ");
  hTPCNSigmas->Draw("HIST SAME ");
  hSigExtBinCount->Draw("HIST SAME ");
  l1->Draw("SAME");
  Sel->SaveAs(Form("imagesoff/%s-SelectionSystematics_V0-%03.0f_%03.0f-V0M-%03.0f_%03.0f.png",Type.Data(),LowMult, HighMult, LowEE, HighEE));
  ...
  //INBUNCH
  TFile* inbunch = TFile::Open(Form("SystematicsPileUp-%sSum.root",Type.Data()));
  ...
  //MATBUDG
  TFile* matbudg  = TFile::Open(Form("systMatBudget-%sSum.root",Type.Data()));
  ...
  //Feeddown
  TFile* fileFD = TFile::Open("FDSyst.root");
  TH1D* hFD = (TH1D*)fileFD->Get("hSystFD");    
  ...
  for (int i = 1; i<= hV0Radius->GetNbinsX(); i++){
    ...
    Double_t oob = hoob->GetBinContent(i); 
    hoob->SetBinContent(i,hoob->GetBinContent(i)); 
    Double_t fdown = hFD->GetBinContent(i);
    Double_t g4 = hG4->GetBinContent(i);


    hSystExtra->SetBinContent(i, 
      TMath::Sqrt( MultIndipEffic*MultIndipEffic + MaterialBudget*MaterialBudget + ibp*ibp + oob*oob + fdown*fdown + g4*g4) 
    );
  }
  ...
  hG4->Draw("HIST SAME");
  lextra->Draw("SAME");
  xlabel-> DrawLatex(0.25, 0.8,Form("%s","#Lambda + #bar{#Lambda}"));
  Extra->SaveAs(Form("imagesoff/ExtraSystematics%s_V0-%03.0f_%03.0f-V0M-%03.0f_%03.0f.png",Type.Data(),LowMult, HighMult, LowEE, HighEE));
  ...
  legend->AddEntry(hSystTopological,"Topological selections","L");
  legend->AddEntry(hSystOthers,"Other selections","L");
  ...
  cn->SaveAs(Form("imagesoff/%s-Systematics-Displayed_V0-%03.0f_%03.0f-V0M-%03.0f_%03.0f.png",Type.Data(),LowMult, HighMult, LowEE, HighEE));
  ...
  hSystTotNeg->Draw("E2");
  xlabel-> DrawLatex(0.25, 0.8,"#Lambda + #bar{#Lambda}");
  cfinal->SaveAs(Form("imagesoff/FinalSystContribPlot%s-%03.0f_%03.0f-V0M-%03.0f_%03.0f.png",Type.Data(),LowMult, HighMult, LowEE, HighEE));
          ...
          TString lWhichSystVar = "V0Radius")
{
  ...
  //Particle
  lDataFilename[0] = Form( "files/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_FDNoFD.root", lCascType.Data(), lMultBoundLo, lMultBoundHi, lEEBoundLo, lEEBoundHi );
  ...
  //Anti Particle
  if (!lType.Contains("K0Short")) {
    lAntiDataFilename[0] = Form("files/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_FDNoFD.root", lAntiCascType.Data(), lMultBoundLo, lMultBoundHi, lEEBoundLo, lEEBoundHi);
