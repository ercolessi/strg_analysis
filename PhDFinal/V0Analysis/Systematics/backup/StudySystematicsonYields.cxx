...
void StudySystematicsonYields(
    ...
    l->Draw("SAME");
    xlabel-> DrawLatex(0.3, 0.82, "#Lambda + #bar{#Lambda}"); 
    ...
    legend->Draw("SAME");  
    xlabel-> DrawLatex(0.8, 0.82, "#Lambda + #bar{#Lambda}"); 
...
TH1F* ReturnMaxDev(
    ...
    for (int nfile = 0; nfile < 4 ; nfile++){
        hMBCutVar[nfile] = (TH1F*)MBinputfilename->Get(Form("%sVariationCuts%s/hYield-%s-%s-%i",fWhichVarEstimator.Data(),fWhichSystVar.Data(),fWhichSystVar.Data(),fWhichVarEstimator.Data(),nfile+1));
