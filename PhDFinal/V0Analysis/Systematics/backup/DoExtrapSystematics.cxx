...
void DoExtrap(
  ...
  Double_t lFixedLo = 0.0,
    ...
    for (int i = 0; i<binnumber; i++){
      if (lCascType.Contains("Xi") || lCascType.Contains("Lambda")) hsyssgnloss[i] = (TH1F*)filesgnlosssyst->Get(Form("var_%.0f-%0.f",percentile[i],percentile[i+1]));
    ...
    if (Func.Contains("BlastWave")) {
      if (lCascType.Contains("Lambda") && fWhichEstimator.Contains("V0M") && lFixedLo == 40. && lFixedHi == 50.){
        max[6] = 6.5;      
       }
    }

    if (Func.Contains("Boltz")) {
      if (lCascType.Contains("Lambda") && fWhichEstimator.Contains("V0M") && lFixedLo == 0. && lFixedHi == 100.){
        max[0] = 1.5;      
       }
       if (lCascType.Contains("Lambda") && fWhichEstimator.Contains("V0M") && lFixedLo == 10. && lFixedHi == 20.){
        max[0] = 2.5;
        max[1] = 2.5;
        max[2] = 2.0;
        max[3] = 2.0;
