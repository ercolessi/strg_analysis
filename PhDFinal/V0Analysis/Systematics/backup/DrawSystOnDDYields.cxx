void DrawSystOnDDYields(
    ...
    for(int i = 1; i<=hCutVar->GetNbinsX();i++){
        htot->SetBinContent(i,TMath::Sqrt(
            hExtrap->GetBinContent(i)*hExtrap->GetBinContent(i) + 
            hCutVar->GetBinContent(i)*hCutVar->GetBinContent(i)));
    ...
    htot->SetLineWidth(3);
    htot->GetYaxis()->SetRangeUser(0.,0.05);
    htot->SetStats(0);
    htot->Draw();
    hCutVar->SetLineWidth(2);
    hExtrapClone->SetLineWidth(2);
    hCutVar->Draw("SAME");
