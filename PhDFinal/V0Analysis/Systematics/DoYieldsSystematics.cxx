...
TH1 *YieldMean(TH1 *hstat, TF1 *f = NULL, Double_t minfit = 0.6, Double_t maxfit = 6.5, TString logfilename = "log.root", Double_t min = 0.6, Double_t max = 10., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q");
    ...
    Bool_t kDoMB = kFALSE)
{
  ...
  // Selection classes
  // TH1F* hCompetingSpecies = ComputeYieldsforSystematics(outputfilename,fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(),lFixedLo,lFixedHi,"CompetingRejection",percentile,percbinnumb);
  ...
  TH1F *hNegToPV = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "DCAPosToPV", percentile, percbinnumb);
  ...
  TH1F *hGeantFluka = 0x0; // ComputeYieldsforSystematics(outputfilename,fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(),lFixedLo,lFixedHi,"GeantFlukaCorrection",percentile,percbinnumb);
  TH1F *hNCr = ComputeYieldsforSystematics(outputfilename, fWhichParticle.Data(), fWhichVarEstimator.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi, "NCrossedOverFindable", percentile, percbinnumb);
...
TH1F* ComputeYieldsforSystematics(
    ...
    for (int i = 0; i < binnumber; i++){ //loop over selection classes
        ...
        //
        lResultsFile[i][0] = TFile::Open(Form("files/%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_FDNoFD.root", fWhichParticle.Data(), fWhichParticle.Data(), spdmin, spdmax, v0min, v0max));
        if (!lCascType.Contains("K0Short")) lAntiResultsFile[i][0] = TFile::Open(Form("files/%s/Results-%s-13TeV-SPDClusters_%03.0f_%03.0f_V0M_%03.0f_%03.0f_FDNoFD.root", fWhichAntiParticle.Data(), fWhichAntiParticle.Data(), spdmin, spdmax, v0min, v0max));
    ...
    // Get the yields
      ...
      for (int j = 0; j < 5; j++){
        ...
        hout[i][j] = (TH1F *)YieldMean(lHistPtClone[i][j], LevyTsallisfunc[i][j], minfit, maxfit, Form("log%s-%s_%03.0f_%03.0f.root", fWhichParticle.Data(), fWhichFixedEstimator.Data(), lFixedLo, lFixedHi));
        // Get yield
