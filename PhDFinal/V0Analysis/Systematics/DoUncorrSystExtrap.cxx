    ...
    Double_t lFixedHi = 50.)
{
  ...
  for (int nmult = 0; nmult < percbinnumb; nmult++)
  {
    ...
    for (int k = 1; k < 5; k++)
    {
      if (nmult == percbinnumb - 1)
        cout << maxdev << "   " << valuexfile[k] << endl;
      maxdev = max(maxdev, valuexfile[k]);
      if (maxdev == valuexfile[k])
        counter = k;
    }
    if (nmult == percbinnumb - 1)
      cout << maxdev << endl;
  ...
  hMaxDevTot->GetYaxis()->SetRangeUser(0., 0.05);
  ...
  YieldCorrSyst[0]->GetYaxis()->SetRangeUser(0., 0.1);
  ...
  xlabel3->DrawLatex(0.7, 0.76, "#Lambda + #bar{#Lambda}");
  ...
  SystFinal[0]->GetYaxis()->SetRangeUser(0.9, 1.1);
  ...
  hMaxDevUncorr->GetYaxis()->SetRangeUser(0., .05);
  ...
  YieldRatioMB[0]->Draw();
  YieldRatioMB[0]->GetYaxis()->SetRangeUser(.9, 1.1);
