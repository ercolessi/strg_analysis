...
void DrawpTShapeCorr(
    ...
    Int_t lClassCode = kVeryLowZN,
    Bool_t DoMB = kFALSE)
{

    TFile* file;
    if (!DoMB){
        file = TFile::Open(Form("../correctedspectra/CorrSpectra-%s-13TeV_SPDClusters_V0M_class%i.root",
                                lCascType.Data(), lClassCode));
    } else {
    ...
    TString lclassname[] = {"kStandalone", "kHighMult", "kLowMult", "kHighZN", "kLowZN"};
    ...
    if (lClassCode == kStandalone) {
        ...
        colors = colors7;
        nbins = 7;
    } else if (lClassCode == kLowZN || lClassCode == kHighZN ) {
        colors = colors5;
        nbins = 5;
    ...
    lab0->DrawLatex(0.7, 0.76, Form("Class %i",lClassCode));

    //
    TLegend* leg = new TLegend(0.25,0.1,0.5,0.4);
    leg->SetTextSize(0.02);
    leg->SetBorderSize(0);
    leg->SetHeader(Form("%s",lclassname[lClassCode].Data()));
    ...
    for (int i = 0; i< percbinnumb; i++){
        leg->AddEntry(hGenSpectraIT0[i],Form("%s", label[i].Data()),"LEP");
    }
    leg->Draw("SAME");
    ...
    if (!DoMB){
    c1->SaveAs(Form("mcptshapeimages/GenSpectraIT0_%s_%i.png", lCascType.Data(), lClassCode));
    c1->SaveAs(Form("mcptshapeimages/GenSpectraIT0_%s_%i.root", lCascType.Data(), lClassCode));
    c1->SaveAs(Form("mcptshapeimages/GenSpectraIT0_%s_%i.pdf", lCascType.Data(), lClassCode));
    } else {
    ...
    lab0->DrawLatex(0.3, 0.86, "Iteration 0");
    labbig->DrawLatex(0.3, 0.79, Form("#%s",lCascType.Data()));
    lab0->DrawLatex(0.7, 0.76, Form("%s",lclassname[lClassCode].Data()));
    leg2->Draw("SAME");

    if (!DoMB){
    c->SaveAs(Form("mcptshapeimages/RatioIT0_%s_class%i.png", lCascType.Data(), lClassCode));
    c->SaveAs(Form("mcptshapeimages/RatioIT0_%s_class%i.root", lCascType.Data(), lClassCode));
    c->SaveAs(Form("mcptshapeimages/RatioIT0_%s_class%i.pdf", lCascType.Data(), lClassCode));
    } else {
    ...
    if (!DoMB){
    cc->SaveAs(Form("mcptshapeimages/RatioIT1_%s_%i.png", lCascType.Data(), lClassCode));
    cc->SaveAs(Form("mcptshapeimages/RatioIT1_%s_%i.root", lCascType.Data(), lClassCode));
    cc->SaveAs(Form("mcptshapeimages/RatioIT1_%s_%i.pdf", lCascType.Data(), lClassCode));
    }
    else{
    ...
    if(!DoMB){
    ccc->SaveAs(Form("mcptshapeimages/RatioIT2_%s_%i.png", lCascType.Data(), lClassCode));
    ccc->SaveAs(Form("mcptshapeimages/RatioIT2_%s_%i.root", lCascType.Data(), lClassCode));
    ccc->SaveAs(Form("mcptshapeimages/RatioIT2_%s_%i.pdf", lCascType.Data(), lClassCode));
    }
    else{
