...
void plotdistributions()
{
    ...
    //Draw stuff
    TH1D *hh = new TH1D("hh", ";#LT n_{ch} #GT w.r.t. to MB (|#eta|<0.5);- #LT ZN #GT w.r.t. MB (|#eta>8|)", 10, 0.0, 3.6);
    hh->SetStats(0);
    hh->GetYaxis()->SetRangeUser(-1.8, -0.1);
    ...
    TCanvas *canvas1 = new TCanvas("canvas1", "", 1100, 900);
    ...
    for (int isel = 0; isel < nbins[kHighMult]; isel++){
        ...
        graph1[isel]->SetMarkerStyle(kFullCircle);
        graph1[isel]->SetMarkerSize(2.8);
    ...
    for (int isel = 0; isel < nbins[kLowMult]; isel++){
        ...
        graph2[isel]->SetMarkerStyle(kOpenCircle);
        graph2[isel]->SetMarkerSize(2.8);
        graph2[isel]->SetMarkerColor(colors[isel]);
        graph2[isel]->Draw("P SAME");
    }

    TLegend *legend1 = new TLegend(0.15, 0.7, 0.35, 0.95);
    ...
    for (int i = 0; i < nbins[kHighMult]; i++) {
        ...
        m1[i]->SetMarkerSize(2.5);
        m1[i]->SetMarkerColor(colors[i]);
        legend1->AddEntry(m1[i], label[i], "p");
    }
    legend1->Draw();

    TLegend *legend2 = new TLegend(0.35, 0.75, 0.5, 0.95);
    ...
    for (int i = 0; i < nbins[kLowMult]; i++) {
        ...
        m2[i]->SetMarkerSize(2.5);
        m2[i]->SetMarkerColor(colors[i]);
        legend2->AddEntry(m2[i], label[i], "p");
    }
    legend2->Draw();

    canvas1->SaveAs("images/fixmult.png");

    TCanvas *canvas2 = new TCanvas("canvas2", "", 1100, 900);
    canvas2->SetBottomMargin(0.12);
    canvas2->SetLeftMargin(0.12);
    canvas2->SetRightMargin(0.02);
    canvas2->SetTopMargin(0.02);
    canvas2->SetTicky();
    canvas2->SetTickx();
    hh->Draw();
    ...
    for (int isel = 0; isel < nbins[kVeryLowZN]; isel++)
    {
        ...
        graph5[isel]->SetMarkerStyle(kFullCross);
        graph5[isel]->SetMarkerSize(2.8);
        graph5[isel]->SetMarkerColor(colors2[isel]);
        graph5[isel]->Draw("P SAME");
    }

    TLegend *legend3 = new TLegend(0.75, 0.18, 0.9, 0.42);
    ...
    for (int i = 0; i < nbins[kHighZN]; i++) {
        ...
        m3[i]->SetMarkerSize(2.5);
        m3[i]->SetMarkerColor(colors2[i]);
        legend3->AddEntry(m3[i], label[i], "p");
    }
    legend3->Draw();

    TLegend *legend4 = new TLegend(0.55, 0.18, 0.7, 0.42);
    legend4->SetBorderSize(0);
    legend4->SetFillColor(0);
    legend4->SetTextSize(0.03);
    legend4->SetHeader("class kLowZN:");

    TMarker *m4[nbins[kLowZN]];
    for (int i = 0; i < nbins[kLowZN]; i++) {
        m4[i] = new TMarker(0, 0, kFullSquare);
        m4[i]->SetMarkerSize(2.5);
        m4[i]->SetMarkerColor(colors2[i]);
        legend4->AddEntry(m4[i], label[i], "p");
    }
    legend4->Draw();

    TLegend *legend5 = new TLegend(0.75, 0.43, 0.9, 0.62);
    legend5->SetBorderSize(0);
    legend5->SetFillColor(0);
    legend5->SetTextSize(0.03);
    legend5->SetHeader("class kVeryLowZN:");

    TMarker *m5[nbins[kVeryLowZN]];
    for (int i = 0; i < nbins[kVeryLowZN]; i++)
    {
        m5[i] = new TMarker(0, 0, kFullCross);
        m5[i]->SetMarkerSize(2.5);
        m5[i]->SetMarkerColor(colors2[i]);
        legend5->AddEntry(m5[i], label[i], "p");
    }
    legend5->Draw();

    canvas2->SaveAs("images/fixzn.png");

    TCanvas *canvas3 = new TCanvas("canvas3", "", 1100, 900);
    canvas3->SetBottomMargin(0.12);
    canvas3->SetLeftMargin(0.12);
    canvas3->SetRightMargin(0.02);
    canvas3->SetTopMargin(0.02);
    canvas3->SetTicky();
    canvas3->SetTickx();
    hh->Draw();
